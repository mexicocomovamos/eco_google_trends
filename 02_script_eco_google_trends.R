...
if(!require("leaftime")) install.packages("leaftime") & require("leaftime")
...
if(!require("pals")) install.packages("pals") &  require("pals")
...
for(i in 1:length(eco_terms)){
  ...
  tempo_df <- tempo[["interest_over_time"]]
  
  eco_df <- bind_rows(eco_df, tempo_df)
...
fiuf <-"Términos económicos seleccionados en Google Trends"
fiuffi <- "Fuente: elaboración de @MexicoComoVamos con información de @googlemexico | @guzmart_"
ggplot(
  eco_df %>% 
  ...
  aes(
    x = as.Date(date),
    y = hits,
    fill = reorder(str_to_title(keyword), -mean_hits)
  )
) + 
  facet_wrap(~ reorder(str_to_title(keyword), -mean_hits), ncol = 1) +
  geom_area(alpha = 0.8) + 
  scale_fill_manual("", values = Turbo(out.colors = 16)) +
  scale_x_date("", date_breaks = "1 month", date_labels = "%b") +
  labs(title= str_wrap(fiuf, width = 75),
       caption = str_wrap(fiuffi, width = 200)) +
  theme_minimal() +
...
ggsave(filename = "eco_google_trends/eco_google_trends.png",
